...
- [x] 쿼리 결과 diff 계산 유틸리티 (`src/utils/resultDiff.ts`)
- [x] 추가/삭제/변경 행 감지 및 셀 단위 비교
- [x] 키 컬럼 자동 감지 (id, _id, code 등)
- [x] Diff WebView 패널 (`src/webview/DiffPanel.ts`) — 통합 뷰
- [x] 상태별 필터 (전체/추가/삭제/변경/동일)
- [x] 마크다운/JSON 내보내기
- [x] 핀된 결과 선택 → diff 실행 명령 (`dbunny.compareResults`)
- [x] 유닛 테스트 14개

### v3.0.0 (2026-04-10)

- [x] 숫자 컬럼용 막대 차트 — SVG 기반, Y축 눈금, 라벨 회전
- [x] 값 분포용 파이 차트 — 슬라이스 + 범례, 퍼센트 표시
- [x] 시계열 데이터용 라인 차트 — 포인트, 라인, 라벨 간격 자동 조정
- [x] 집계 옵션 (원본 / 합계 / 개수 / 평균) — 라벨 컬럼별 그룹 집계
- [x] 차트 빌더 유틸리티 신규 (`src/utils/chartBuilder.ts`)
- [x] 차트 WebView 패널 신규 (`src/webview/ChartPanel.ts`)

### v2.9.0 (2026-04-04)

- [x] 컬럼별 최솟값 / 최댓값 / 평균 — 숫자/문자 타입 모두 지원
- [x] NULL 비율 및 고유값(distinct) 개수 — 시각적 NULL 바 표시
- [x] 데이터 분포 요약 — 요약 카드 (총 행 수, 컬럼 수)
- [x] 상위 N개 빈출값 — 컬럼별 Top 5 빈출값 바 차트
- [x] 데이터 통계 유틸리티 신규 (`src/utils/dataStatistics.ts`)
- [x] 통계 WebView 패널 신규 (`src/webview/StatisticsPanel.ts`)

### v2.8.0 (2026-03-28)

- [x] 필드명과 타입이 표시되는 폼 기반 행 삽입
- [x] 컬럼 제약 조건 자동 감지 (NOT NULL, DEFAULT, FK) — 배지로 시각 표시
- [x] FK 참조 값 드롭다운 선택 — 참조 테이블에서 최대 200개 값 조회
- [x] CSV 붙여넣기 일괄 삽입 — 헤더 행 기반 컬럼 매핑, 미리보기 지원
- [x] 행 삽입 WebView 패널 신규 (`src/webview/RowInsertPanel.ts`)

### v2.7.0 (2026-03-22)

- [x] .xlsx 형식 지원 — SheetJS(xlsx) 기반 Excel 내보내기
- [x] 멀티시트 내보내기 — 데이터베이스 우클릭 시 모든 테이블을 시트별로 내보내기
- [x] 헤더 행에 컬럼 타입 표시 — PK, NOT NULL, DEFAULT 등 스키마 정보 포함
- [x] 컬럼 너비 자동 조정 — 데이터 길이 기반 (한글 wide char 고려)
- [x] Excel 내보내기 유틸리티 신규 (`src/utils/dataExport.ts`)
- [x] 쿼리 결과 패널에서 Excel 내보내기 지원

### v2.6.0 (2026-03-22)

- [x] CSV, JSON, Excel(.xlsx) → 테이블로 직접 가져오기
- [x] 컬럼 매핑 미리보기 화면 — 소스-타겟 자동 매핑 제안, 수동 변경 가능
- [x] 충돌 처리 옵션 (건너뛰기 / 덮어쓰기 / Upsert) — DB별 적절한 SQL 생성
- [x] 대용량 파일 진행률 표시 — 배치 단위 진행률 바 + 삽입/건너뜀/실패 실시간 카운트
- [x] 데이터 가져오기 유틸리티 신규 (`src/utils/dataImport.ts`)
- [x] 데이터 가져오기 WebView 패널 신규 (`src/webview/DataImportPanel.ts`)

### v2.5.0 (2026-03-20)

- [x] 기존 연결 원클릭 복제 — 컨텍스트 메뉴에서 암호화된 비밀번호 포함 즉시 복제
- [x] 연결 설정 JSON 내보내기 (비밀번호 제외) — 단일/전체 내보내기, `.dbunny.json` 봉투 형식
- [x] 공유받은 연결 설정 가져오기 — JSON 유효성 검증 후 일괄 추가
- [x] 팀용 연결 템플릿 — 저장/생성/삭제, 최대 50개, QuickPick 선택 → 폼 프리필
- [x] 연결 공유 유틸리티 신규 (`src/utils/connectionShare.ts`)
- [x] 유닛 테스트 36개

### v2.4.0 (2026-03-17)

- [x] 연결에 색상 지정 — 8가지 프리셋 컬러 (빨강=운영, 주황=스테이징, 노랑=테스트, 초록=개발, 파랑=로컬, 보라=분석, 핑크, 회색)
- [x] 연결 폼에 컬러 피커 UI — 원형 색상 스와치, 사용자 지정 라벨
- [x] 트리뷰 컬러 인디케이터 — 연결 아이콘 색상 + 컬러 도트
- [x] 탭 바 컬러 바 — 쿼리 탭 좌측에 연결 색상 표시
- [x] 연결 배지 컬러링 — 툴바 연결 배지에 색상 반영
- [x] 상태 바 컬러 — 연결 색상 + 라벨 표시
- [x] 운영 환경 경고 배너 — 빨간색(운영) 연결 시 "PRODUCTION" 경고 표시
- [x] 유닛 테스트 66개 + 통합 테스트 20개
