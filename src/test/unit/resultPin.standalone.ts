...
// unpinResult도 불변성 유지
const beforeUnpin = { ...modified, pinnedResults: [...modified.pinnedResults] };
...
let dataState = pinResult(createDefaultTabPinState(), resultData);
...
// 빈 결과 핀
let emptyState = pinResult(createDefaultTabPinState(), createMockResult({
    rows: [],
    rowCount: 0,
    columns: []
}));
...
// 한국어 쿼리
let koreanState = pinResult(createDefaultTabPinState(), createMockResult({
    query: 'SELECT * FROM 사용자 WHERE 이름 = "홍길동"'
}));
assert(koreanState.pinnedResults[0].query.includes('사용자'), '한국어 쿼리 보존');

// 특수문자 포함 쿼리
let specialState = pinResult(createDefaultTabPinState(), createMockResult({
...
let nullDbState = pinResult(createDefaultTabPinState(), createMockResult({
    databaseName: null
}));
...
let longState = pinResult(createDefaultTabPinState(), createMockResult({ query: longQuery }));
