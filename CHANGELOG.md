...
- **Excel Export**: Export table data and query results as `.xlsx` files
  - Single table export from context menu (right-click table → Export Data)
  - Multi-sheet export from database node (all tables as separate sheets)
  - Column type row with PK, NOT NULL, DEFAULT annotations
  - Auto-adjusted column widths (handles wide characters like Korean)
  - Query result panel: export as Excel alongside CSV/JSON
- **Export Utility**: New `src/utils/dataExport.ts` with `exportSingleSheet`, `exportToExcel`, `fetchAndExportTables`
